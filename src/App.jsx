...
export default function App() {
  ...
  const [selected, setSelected] = useState(["person", "car", "dog"]);
    ...
    <div style={{ fontFamily: 'Inter, sans-serif', minHeight: '100vh', background: '#edf2f7', padding: 30 }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        ...
        <p style={{ color: '#4a5568', textAlign: 'center', marginBottom: 30 }}>
          Real-time detection via webcam or image upload with confidence & class filters
        </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 20, boxShadow: '0 6px 20px rgba(0,0,0,0.08)' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 18 }}>
              ...
              <button style={btn('outline')} onClick={() => { if (annotatedSrc) downloadDataURL(annotatedSrc); }}>💾 Save Snapshot</button>
            </div>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 20, boxShadow: '0 6px 20px rgba(0,0,0,0.08)' }}>
            ...
            {/* Class Filter */}
            <div>
              <span style={{ color: '#4a5568', fontSize: 14 }}>Class Filter:</span>
              <div style={{
                display: 'grid',
                ...
                {classes.map((c) => (
                  <label key={c} style={{
                    ...
                    padding: '4px 6px', borderRadius: 6, background: selected.includes(c) ? '#3b82f6' : '#fff',
                    color: selected.includes(c) ? '#fff' : '#1a202c', fontWeight: selected.includes(c) ? 600 : 400
                  }}>
                    <input type="checkbox" checked={selected.includes(c)} onChange={() => toggleClass(c)} style={{ cursor: 'pointer' }} />
                    {c}
                  </label>
                ))}
              </div>
...
function btn(variant = 'primary') {
  const base = { padding: '10px 18px', borderRadius: 10, border: '1px solid transparent', cursor: 'pointer', fontWeight: 600, fontSize: 14, transition: '0.25s', display: 'inline-flex', alignItems: 'center', gap: 6 };
